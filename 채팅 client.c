...
void ErrorHandling(char *message); //예외처리 함수
...
int main(int argc, char **argv) {
    ...
    int nRcv;    
    unsigned int Addr;
    char message[BUFSIZE];   // 메세지의 배열 선언
      
    struct hostent *host;
          
    printf("Server Address : ");       gets(sAddr);  //server ip주소값 입력
    printf("Port Number : ");       gets(message); //포트번호 입력
    sPort = atoi(message);
    ...
    if(connect(clntSock, (void *)&servAddr, sizeof(servAddr)) == SOCKET_ERROR) {
        ...
        printf("Connect OK!\nStart...\n");  //예외처리 if문에 해당되지 않을때 연결완료 문자 출력
    }
  
    while(1) {
        printf("\nSend Message : ");
        gets(message);   //보낼 메세지 입력
          
        if(strcmp(message, "exit") == 0) {  //만약 메세지 입력이 exit이면
